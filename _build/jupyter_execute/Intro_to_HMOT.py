...
# $$
# 
# Here are instructions for obtaining the eigenvalues and eigenvectors using the `numpy.linalg.eig()` function.
# 
# ## Applying the Method
# 
...
# ### Step 1
# 
# Step one is to number the atoms in your &pi; system. We have numbered the carbons of butadiene like so.
# 
...
# ### Step 2
# 
# We now must translate the interaction matrix in a **data struture** that we can manipulate in *Python*. We will be using a *NumPy* **array**. 
# 
# Recall that we has set $x = \frac{\alpha – E}{\beta}$ and that will still be true here. However, the `eig()` function that we will be using can only work with numbers. The letter "*x*" will cause errors. We can avoid trouble by seting a **numeric** value for x. In practice, this will be to set the value of *x* to zero. The result of this statement is that we are effectively setting the start value of &alpha; to **zero**. This is easy to justify as we are saying that each atomic *p*-orbital starts at the same **relative energy** and that will be true if they are all carbon atoms.
# 
# So for the individual atoms we state that&hellip;
# 
# $$
# \begin{align*}
# 0 & = \frac{\alpha – E}{\beta} \\
...
# $$
# 
# We now have the following **diagonal matrix**. The eigenvalues of this matrix with correspond to the **relative energy** values above and below &alpha; That energy will be the eigenvalue multiplied by &beta;
# 
# $$
# \begin{bmatrix}
#    0        & 1      & 0      &  0 \\
...
# $$
# 
# As a **quick rule**, we could say that we construct an interaction matrix by **starting** with a matrix of all zeros and inserting the value of one for all **connections** between carbon atoms. It's that easy.
# 
...
# ```{note}
# In the code above you will see that I flipped the signs of the eigenvalues. This is because the energy was defined earlier as $E = \alpha -x \cdot \beta$. Note the negative sign for the relative energy term, $-x \cdot \beta$ A positive value is actually the lowest energy. I fell that it is easier to interpret if I apply that negative sign to the eigenvalues. This is why using a notebook like this to document your procedure is important. Now you know where the negative sign came from.
# ```
# 
# ## Processing the Results
# 
# the algorith found the eigenvalues and their corresponding eigenvectors. It did not care about the order though. The code below will sort the eigenvalues in order from lowest energy to highest and then sort the eigenvectors to match.
...
# ## Calculated Results
# We can make a visual diagram as above by inspecting the energies and the coefficients at each energy (the eigenvalues and eigenvectors). We can also get quantitative information from the eigenvector matrix about electron desnsity, bond orders, free valence values and more. Its just math.
# 
# The first important information we need is orbital occupancy. Butadiene has four electrons in the &pi; system. The first two orbitals are occupied and the final two are not, as seen in the diagram above. We can express this a an array of values.
...
# We now know where the electrons are and we havew calculated the geometry of each molecular orbital. This will allow use to calculate properties that arise from electrons and molecular orbitals together.

# ### Electron Density
# Foe each atom, the electron density is the square of the atomic *p*-orbital coefficient multiplied by number of electrons in each orbital. 
# 
...
# 
# Applying a square function to the who matrix was easy in the case of electron density but now we would need to write a small amount of code to iterate through the coefficients and perform this calculation. I'm not going to do that because we will soon be using a package of code that does all this for us. It was written by someone else and I will not be reinventing that wheel.
# 
...
# ## Summary
# 
# We have explored constructing an interaction matrix as a *NumPy* array and using the eig() unction to obtain the eigenvalues and eigenvectors. We have demonstrated this with butadiene and used the results to calculate the electron desnity ande partial charge at each atom.
# 
